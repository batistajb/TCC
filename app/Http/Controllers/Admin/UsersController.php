...
class UsersController extends Controller {
	...
	public function store( Request $request ) {
		...
		if ( $request->password != $request->pass2 ) {
			return redirect()->back()->withErrors( $validation->errors() )->withInput( $request->all() )->with( 'status', 'Senhas não conferem!' );
		}
		if ( $validation->fails() ) {
			return redirect()->back()->withErrors( $validation->errors() )->withInput( $request->all() );
		}
	...
	public function editProfile( Request $request ) {
		...
		if ( $user != null ) {
			if ( $request->password != $request->pass2 ) {
				return redirect()->back()->withErrors( $validation->errors() )->withInput( $request->all( 'name', 'email', 'cpf' ) )->with( 'status', 'Senhas não conferem!' );
			}
		...
		if ( $update ) {
			return redirect()->route( 'dashboard' )->with( 'status', 'Usuário alterado com sucesso!' );
		}

		return redirect()->route( 'myProfile' )->with( 'status', 'Error ao alterar!' );
	...
	public function searchUsers( Request $request ) {
		...
		$user = User::all( )->where( 'id', '=', $request->users )->first();

		if ( empty( $user ) ) {
			return back()->with( 'status', 'Usuário não cadastrado!' );
		} else {
